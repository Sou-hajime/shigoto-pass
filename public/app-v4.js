...
function menuModal(){return `<div class="modal-backdrop"><section class="modal menu-modal" role="dialog" aria-modal="true" aria-label="メニュー"><button class="modal-close" aria-label="閉じる">×</button><h2>メニュー</h2><button data-scroll="services">対応できる仕事</button><button data-scroll="managed">ご利用の流れ</button><button data-scroll="quality">安心への取り組み</button><button data-scroll="network">事業所の方へ</button><button data-action="consult">仕事を相談する</button></section></div>`}
